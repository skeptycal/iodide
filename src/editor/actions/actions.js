...
export function makeNotebookCopy(forkedFrom) {
  return async dispatch => {
    try {
      await createNewNotebookOnServer(forkedFrom);
      dispatch(
        updateAppMessages({
          message: "Made copy of notebook.",
...
export function loginSuccess(userData) {
  return (dispatch, getState) => {
    ...
    if (notebookIsATrial(getState())) {
      ...
      // save any pending changes to server immediately
